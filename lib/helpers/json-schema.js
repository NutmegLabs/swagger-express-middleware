...
var ajv = new Ajv({removeAdditional: true, schemaId: 'id', validateSchema: 'log'});
ajv.addMetaSchema(require('ajv/lib/refs/json-schema-draft-04.json'));

// Match everything non-empty for built-in formats because they are validated separately.
ajv.addFormat('int32', '.*');
ajv.addFormat('int64', '.*');
ajv.addFormat('byte', '.*');
ajv.addFormat('float', '.*');
...
function getValueToValidate(schema, value) {
  ...
  if (value === undefined || value === '' ||
    ...
    // (kumandroo): we use examples only for documentation in our Swagger
    /*
...
function jsonValidate(schema, value, propPath) {
  ...
  else if (typeof propPath !== 'undefined') {
    var dataPath = propPath ? propPath + '.' + ajv.errors[0].dataPath : ajv.errors[0].dataPath;
    var e = new Error(ajv.errorsText());
    throw ono(e, {status: 400}, 'JSON Schema validation error. \nData path: "%s" \nSchema path: "%s"',
      dataPath, ajv.errors[0].schemaPath);
  }
  else {
    var e = new Error(ajv.errorsText());
    throw ono(e, {status: 400}, 'JSON Schema validation error.');
