...
function mockResponseBody(req, res, next) {
  if (res.swagger) {
    if (res.swagger.isEmpty) {
      ...
      // Special case: if schema is empty ({}) and content-type suggests text, send as text
      if (res.swagger.schema.type === undefined && 
          _.isEmpty(res.swagger.schema.properties) && 
          _.isEmpty(res.swagger.schema.items) &&
          (req.get('Content-Type') || '').toLowerCase().includes('text')) {
