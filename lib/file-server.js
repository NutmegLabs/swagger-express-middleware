...
function fileServer(context, router, options) {
  ...
  var baseDir = '';
  if (context.parser && context.parser.$refs && context.parser.$refs.paths().length > 0) {
    baseDir = path.dirname(decodeURI(context.parser.$refs.paths()[0]));
  }
  ...
  function serveRawSwaggerFiles(req, res, next) {
    if (req.method === 'GET' || req.method === 'HEAD') {
      ...
      // Check if the request path starts with our config path (normal case)
      // OR if it might be a normalized path from a relative reference (e.g., /api-docs/../pet -> /pet)
      var isNormalPath = _.startsWith(reqPath, configPath);
      var isPossiblyNormalizedPath = !isNormalPath && reqPath.indexOf('/') === 0;
      
      if ((isNormalPath || isPossiblyNormalizedPath) && context.parser && context.parser.$refs) {
        // Get the normalized path of the requested file, relative to the baseDir
        var relativePath;
        if (isNormalPath) {
          // Normal case: /api-docs/file -> file
          relativePath = req.path.substring(configPath.length);
          if (_.startsWith(relativePath, '/')) {
            relativePath = relativePath.substring(1);
          }
        } else {
          // Normalized case: /pet (from /api-docs/../pet)
          relativePath = req.path.substring(1); // Remove leading /
        }
        relativePath = path.normalize(util.normalizePath(relativePath, router));

        // See if any of the Swagger files match this path
        var filePath = _.find(context.parser.$refs.paths(), function(file) {
          var relativeFile = path.relative(baseDir, file);
          ...
          // Special case for normalized paths: if we're looking for "pet" and have "../pet",
          // resolve the relative path and check if it matches
          if (isPossiblyNormalizedPath) {
            var resolvedFile = path.resolve(baseDir, relativeFile);
            var resolvedRequest = path.resolve(baseDir, '..', relativePath);
            if (resolvedFile === resolvedRequest) {
              return true;
            }
          }
