...
function Middleware(sharedRouter) {
  ...
  this.init = function(swagger, callback) {
    ...
    // First resolve to get $refs, then dereference to get the API
    SwaggerParser.resolve(swagger, function(err, $refs) {
      if (err) {
        util.warn(err);
        context.error  = err;
        context.api    = null;
      ...
      // Now dereference to get the dereferenced API
      SwaggerParser.dereference(swagger, function(err2, api) {
        if (err2) {
          util.warn(err2);
        }
        ...
        context.error  = err2;
        context.api    = api;
        // Create a parser-like object with $refs for backward compatibility
        context.parser = $refs ? { $refs: $refs } : null;
        context.emit('change');

        if (_.isFunction(callback)) {
          callback(err2, self, context.api, context.parser);
        }
      });
    });
  };
