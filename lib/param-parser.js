  ...
  getParams(req).forEach(function(param) {
    if (param.in === 'formData') {
      ...
      if (param.type === 'file') {
        ...
        // Initialize req.files if it doesn't exist
        if (!req.files) {
          req.files = {};
        }
        ...
        // Initialize req.body as object if it's not already an object
...
function parseBodyParam(req, res, next) {
  ...
  params.some(function(param) {
    if (param.in === 'body') {
      ...
      // For required body parameters with non-string schema, treat empty string as undefined
      // (Empty strings are valid for string-type schemas)
      if (param.required && req.body === '' && 
          !(param.type === 'string' || (param.schema && param.schema.type === 'string'))) {
        req.body = undefined;
      }
